...
%ABRIR ARCHIVO 
archivo=fopen('C:\Users\karla\Downloads\TeoriadeLaInfo\P1\Libro_FRA.txt','r');
...
ABC=['a','b','c','ç','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z',' ']; 
...
xlabel('Caracteres');
ylabel('Probabilidad');
title('Karla Robert - Distribución de Probabilidad de Caracteres');
grid on;
...
title('Karla Robert - Cantidad de Información por Caracter');
grid on;
% --- mostrar valores encima ---
for i=1:length(I1)
...
if topN > 0
    ...
    figure;
    bar(double(parejas_ordenadas(1:topN)), 'FaceColor', [1 0 1]);
...
if topM > 0
    ...
    figure;
    bar(double(tercias_ordenadas(1:topM)), 'FaceColor', [0 1 1]); % naranja
...
bar(hs, 'FaceColor', [0.6 0.2 0.6]); % morado
